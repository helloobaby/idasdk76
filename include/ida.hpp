...
{
  return getinf_buf(INF_PROCNAME, buf, bufsize);
}
...
{
  return getinf_buf(INF_STRLIT_PREF, buf, bufsize);
}
...
inline bool inf_get_cc(compiler_info_t *out) { return getinf_buf(INF_CC, out, sizeof(*out)); }
inline bool inf_set_cc(const compiler_info_t &_v) { return setinf_buf(INF_CC, &_v, sizeof(_v)); }

inline bool inf_get_privrange(range_t *out) { return getinf_buf(INF_PRIVRANGE, out, sizeof(*out)); }
