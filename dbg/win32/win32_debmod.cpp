...
struct regctx_t : public regctx_base_t
{
  ...
  bool init(void);
  bool load(context_holder_t *out);
  bool store(const context_holder_t &ctxh);
};
...
//--------------------------------------------------------------------------
bool regctx_t::load(context_holder_t *out)
{
  return init() && debmod.get_thread_state(out, &cpu, &fpu, tid, clsmask);
}

//--------------------------------------------------------------------------
bool regctx_t::store(const context_holder_t &ctxh)
{
        ...
        const context_helper_t &helper)
{
  ...
  if ( xstate_ctx != NULL && helper.xstate_helpers_loaded() )
...
#ifndef __X86__
  if ( is_wow64() && _Wow64GetThreadContext != NULL )
  {
...
{
#ifndef __X86__
  if ( is_wow64() && _Wow64SetThreadContext != NULL )
  {
    context_holder_t xstate_ctx;
    if ( (clsmask & X86_RC_YMM) != 0 )
    {
        ...
        qstring * /*errbuf*/)
{
  ...
  thread_info_t *ti = threads.get(tid);
  context_holder_t ctxh;
  if ( ti != NULL && ti->read_context(&ctxh, X86_RC_SEGMENTS) )
  {
        ...
        qstring * /*errbuf*/)
{
  ...
  context_holder_t ctxh;
  if ( !reg_ctx->load(&ctxh) )
    return DRC_FAILED;

  if ( !reg_ctx->patch(reg_idx, value) )
    return DRC_FAILED;

  if ( !reg_ctx->store(ctxh) )
    return DRC_FAILED;
