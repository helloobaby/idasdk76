...
static uint32 mt_state = MT_NONE;
...
{
#ifdef __ANDROID__ // android passes NULL as hproc, do not use it
  qnotused(hproc);
...
{
  errno = 0;
  if ( qptrace(PTRACE_TRACEME, 0, NULL, NULL) != 0 )
  {
...
static void linux_test_for_tracefork(void)
{
  ...
  ret = qptrace(PTRACE_SETOPTIONS, child_pid, NULL, (void *)PTRACE_O_TRACEFORK);
  if ( ret != 0 )
  {
  ...
  ret = qptrace(PTRACE_CONT, child_pid, NULL, NULL);
  if ( ret != 0 )
  ...
  {
    long second_pid = 0;
    ret = qptrace(PTRACE_GETEVENTMSG, child_pid, NULL, &second_pid);
...
static void linux_enable_event_reporting(int pid)
{
  ...
  qptrace(PTRACE_SETOPTIONS, pid, NULL, (void *)PTRACE_O_TRACECLONE);
}
