...
// Pointers to imported functions libunwind-x86_64.so.8
...
// address space for libunwind
  ...
  load_libunwind_status_t uwn_status;
...
//-------------------------------------------------------------------------
void libunwind_access_t::init()
{
  if ( uwn_status != UNW_UKNOWN )
    return;
  qstring buf;
  if ( !qgetenv("IDA_USE_LIBUNWIND", &buf) )
  {
    uwn_status = UNW_NOT_ALLOWED;
    return;
  }
  if ( !load_lib_so(&libx86_64, unwsyms, qnumber(unwsyms), "libunwind-x86_64.so.8") )
  {
    uwn_status = UNW_NOT_ALLOWED;
    return;
  }
  if ( !load_lib_so(&libptrace, ptracesyms, qnumber(ptracesyms), "libunwind-ptrace.so.0") )
  {
    close_lib(&libx86_64, unwsyms, qnumber(unwsyms));
    uwn_status = UNW_NOT_ALLOWED;
    return;
  ...
  msg("Loaded libunwind-x86_64.so.8 and libunwind-ptrace.so.0\n");
  uwn_status = UNW_LOADED;
}
...
#define unw_x64_init_remote         p_libaccess->p_unw_init_remote
#define unw_x64_step                p_libaccess->p_unw_step
#define unw_x64_get_reg             p_libaccess->p_unw_get_reg
#define unw_ptrace_create           p_libaccess->p_ptrace_create
#define unw_ptrace_destroy          p_libaccess->p_ptrace_destroy
#define uwn_as                      p_libaccess->as
...
drc_t idaapi linux_debmod_t::dbg_update_call_stack(thid_t tid, call_stack_t * trace)
{
  ...
  if ( p_libaccess->uwn_status == libunwind_access_t::UNW_UKNOWN )
    p_libaccess->init();
  if ( p_libaccess->uwn_status == libunwind_access_t::UNW_NOT_ALLOWED )
    return DRC_NONE;
  void *context = unw_ptrace_create(tid);
  unw_cursor_t cursor;
  if ( unw_x64_init_remote(&cursor, uwn_as, context) != 0 )
    return DRC_FAILED;

  do
  {
    unw_word_t rip;
    if ( unw_x64_get_reg(&cursor, UNW_REG_IP, &rip) )
      return DRC_FAILED;
  ...
  while ( unw_x64_step(&cursor) > 0 );
  unw_ptrace_destroy(context);
  return DRC_OK;
}
