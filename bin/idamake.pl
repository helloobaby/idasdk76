...
sub simplify_command_line
{
  ...
  if ( $cmd =~ /^ar /
    || $cmd =~ /^([\.\/]*third_party\/linaro\/(linux|win32)\/arm-linux-gnueabi\/bin\/arm-linux-gnueabi-ar(\.exe)?) /
  ...
  if ( $cmd =~ /^strip /
    || $cmd =~ /^([\.\/]*third_party\/linaro\/(linux|win32)\/arm-linux-gnueabi\/bin\/arm-linux-gnueabi-strip(\.exe)?) /
  ...
  my $compiling = $argv0 =~ m#^/opt/osxcross/bin/(i386|x86_64)-apple-darwin17-[c\+]{2}#
               || $argv0 =~ m#^(\.\./)*third_party/linaro/(linux|win32)/arm-linux-gnueabi/bin/arm-linux-gnueabi-g?[c\+]{2}(\.exe)?#
...
sub print_filtered_gcc_output
{
  ...
  while ( <$FP> )
  {
    ...
    if ( /^(\e\[\d+(;\d+)*m)*(compile|compiling|link|linking|asm|lib|qmake)(\e\[\d+(;\d+)*m)* /
      ...
      || /^(g(\+\+|cc)|cp|[\.\/]*third_party\/lsb\/lsb-build-[0-9\.\-]*\/bin\/lsb(c\+\+|cc)|rm|qcp\.sh|g?make\d*\[\d\]:|ar|moc|uic|rcc|name|strip|mkdeb|perl|ccache|#) /
      || /^([\.\/]*third_party\/linaro\/(linux|win32)\/arm-linux-gnueabi\/bin\/arm-linux-gnueabi-(ar|g(\+\+|cc))(\.exe)?) /
...
#--------------------------------------------------------------------------
GetOptions(\%opt, "v+","Z+", "z+", "make=s") or die;
if ( exists($opt{z}) )
{
